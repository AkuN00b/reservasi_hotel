...
<div class="content-wrapper">
    <div class="container">
        ...
        <div class="row">
          <div class="col-lg-6">
            <form class="forms-sample border-right mb-4 pr-4" method="POST" action="{{ route('receptionist.profile.update') }}">
              @csrf
              @method('PUT')
          ...
          <div class="col-lg-6">
            <form action="{{ route('receptionist.password.update') }}" class="forms-sample mb-4" method="POST">
              @csrf
              @method('PUT')
            ...
            <form action="{{ route('receptionist.image.update') }}" class="forms-sample" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <h4 class="mb-4" style="text-decoration: underline;">Update Image Account</h4>
                <div class="form-group">
                ...
                @if (Auth::user()->image == 'default.png')
                  <img src="{{ asset('storage/account/base/'.Auth::user()->image) }}" alt="Gambar {{ Auth::user()->name }}" style="padding: 0;
                  display: block;
                  margin: 0 auto;
                  max-height: 100%;
                  max-width: 100%;"><br><br>
                @else 
                  <img src="{{ asset('storage/account/'.Auth::user()->image) }}" alt="Gambar {{ Auth::user()->name }}" style="padding: 0;
                  display: block;
                  margin: 0 auto;
                  max-height: 100%;
                  max-width: 100%;"><br><br>
