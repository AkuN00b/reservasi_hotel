            ...
            <div class="card-body">
                <div class="row mb-4">
                    <div class="col-sm-12">
                        ...
                        @if ($bookings->status == 0)
                            <form class="forms-sample" method="POST" action="{{ route('receptionist.booking.roomupdate',$bookings->id) }}">
                                @csrf
                                @method('PUT')
                        ...
                        @elseif ($bookings->image == !NULL && $bookings->status == 5) 
                            ...
                            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#image">
                                View Image
                            </button>
                                ...
                                <td>
                                    @if ($bookings->status == 0)
                                        @if ($bookings->room_number_id == NULL)
                                            ...
                                            <form id="buying-form-{{ $bookings->id }}" action="{{ route('receptionist.booking.buying',$bookings->id.'/'.$bookings->room_number_id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('PUT')
                                            </form>
                                    ...
                                    @elseif ($bookings->status == 1)
                                        ...
                                        <form id="checkin-form-{{ $bookings->id }}" action="{{ route('receptionist.booking.checkin',$bookings->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    @elseif ($bookings->status == 2)
                                    ...
                                    @elseif ($bookings->status == 3)
                                        ...
                                        <form id="checkout-form-{{ $bookings->id }}" action="{{ route('receptionist.booking.checkout',$bookings->id.'/'.$bookings->room_number_id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    @elseif ($bookings->status == 4)
                                    ...
                                    @elseif ($bookings->status == 5)
                                        @if ($bookings->image == NULL) 
                                            ...
                                            <form id="approval-form-{{ $bookings->id }}" action="{{ route('receptionist.booking.approve',$bookings->id.'/'.$bookings->room_number_id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('PUT')
                                            </form>
                                            ...
                                            <form id="decline-form-{{ $bookings->id }}" action="{{ route('receptionist.booking.decline',$bookings->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('PUT')
                                            </form>
                                        @elseif ($bookings->image == !NULL)
                                            ...
                                            <form id="approval-form-{{ $bookings->id }}" action="{{ route('receptionist.booking.approve',$bookings->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('PUT')
                                            </form>
                                            ...
                                            <form id="decline-form-{{ $bookings->id }}" action="{{ route('receptionist.booking.decline',$bookings->id.'/'.$bookings->room_number_id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('PUT')
                                            </form>
                                        @endif
